...
def create_splash(splash_width, splash_height, output_filename):
    # The SVG logo as a multi-line string.
...
"""
    ...
    # Open the resulting PNG as a Pillow Image.
    logo_image = Image.open(io.BytesIO(png_data)).convert("RGBA")

    # Calculate the target size for the logo.
    # The logo should take up one-third of the splash screen width.
    target_width = splash_width // 3
    scale_factor = target_width / logo_image.width
    target_height = int(logo_image.height * scale_factor)

    # Resize the logo with anti-aliasing.
    logo_image = logo_image.resize((target_width, target_height), Image.Resampling.LANCZOS)

    # Create the background splash image with a black background.
    splash = Image.new("RGB", (splash_width, splash_height), "black")

    # Calculate position to center the logo.
    x = (splash_width - target_width) // 2
    y = (splash_height - target_height) // 2

    # Paste the logo onto the splash image, preserving transparency.
    splash.paste(logo_image, (x, y), logo_image)

    # Save the final image.
    splash.save(output_filename)
    print(f"{output_filename} created with resolution {splash_width}x{splash_height}.")

def main():
    # Define the list of resolutions as strings.
    resolutions = [
        "1920x1152",
        "1920x1080",
        "1280x960",
    ...
    # Generate splash image for each resolution.
    for res in resolutions:
        try:
            ...
            print(f"Skipping invalid resolution '{res}': {e}")
            continue

        output_filename = f"splash-{width}x{height}.png"
        create_splash(width, height, output_filename)
